...
namespace PROYECTO.Gramatica.Entorno.Condicional
{
    class Si : IEntorno
    {
        ...
        private bool Exec(ParseTreeNode FuncTree, Operar operar)
        {
            ...
            {
                switch (FuncTree.ChildNodes.Count)
                {
                    case 2:
                        //No tiene else
                        if (new Si(FuncTree.ChildNodes[0], FuncTree.ChildNodes[1], this).Ejecutar())
                        {
                            return true;
                        }
                        break;
                    case 3:
                        if (new Si(FuncTree.ChildNodes[0], FuncTree.ChildNodes[1], FuncTree.ChildNodes[2], this).Ejecutar())
                        {
                            return true;
                        }
                        //Tiene un else
                        ...
                        case "DECLARACION":
                            ...
                            if (dataType >= Tipo.INT && dataType <= Tipo.CLASE)
                            {
                                foreach (var varlst in ramaAux[1].ChildNodes)
                                {
                                    ...
                                    if (IfSym.ContainsKey(varlst.Token.Text.ToLower()))
                                    {
                                    ...
                                    if (!ramaAux[ramaAux.Count - 1].Term.Name.Equals("OPER"))
                                    {
                                        ...
                                        this.IfSym.Add(varlst.Token.Text.ToLower(), new Simbolo(pos, false, null, dataType));
                                    }
                                    else
                                    {
                                        //Opera el oper
                                        ...
                                        IfSym.Add(varlst.Token.Text.ToLower(), ArrSym); Main.Imprimir(String.Format("La variable {0} ya ha sido declarada en este entorno: ({1},{2})", varlst.Token.Text.ToLower(), varlst.Token.Location.Line + 1, varlst.Token.Location.Column + 1));
                                    }
                                }
                            }
                            //Es un ARRAY
                            if (dataType >= Tipo.INTARR && dataType <= Tipo.CLASEARR)
                            {
                                ...
                                foreach (var varlst in ramaAux[1].ChildNodes)
                                {
                                    ...
                                    if (ramaAux.Count == 3)
                                    {
                                        ...
                                        //Añade el simbolo al entorno local
                                        IfSym.Add(varlst.Token.Text.ToLower(), ArrSym);
                                    }
                                    else
                                    {
                                        //Opera el OPER
        ...
        public bool Ejecutar()
        {
            ...
            {
                //Opera la condición
                var symIfOper = operar.Interpretar(this.Condicion);
                //Se asegura que sea un booleano
                if (symIfOper.TipoDato!=Tipo.BOOLEAN)
                {
                    Main.Imprimir(String.Format("Se esperaba un booleano {0}, ({1},{2})",this.ToString(),symIfOper.Posicion != null ? symIfOper.Posicion.Fila :0, symIfOper.Posicion != null ? symIfOper.Posicion.Columna : 0));
                    return true;
                }
                //Ejecuta el nodo if
                if ((bool)symIfOper.Dato)
                {
                    Exec(this.IfTree, operar);
                }
                else
                {
                    //Verifica si hay un else
                    if (this.ElseTree != null)
                    {
                        Exec(this.ElseTree, operar);
                    }
                }
