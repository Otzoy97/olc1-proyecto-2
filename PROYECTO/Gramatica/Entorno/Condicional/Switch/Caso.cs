...
namespace PROYECTO.Gramatica.Entorno.Condicional.Switch
{
    class Caso : Comprobar
    {
        ...
        public Caso(ParseTreeNode acciones, Comprobar padre) :base (padre.Variable,padre.CaseNode,padre.EntornoPadre)
        {
        ...
        public new bool Ejecutar()
        {
            ...
            try
            {
                foreach (var nodeTree in CaseTree.ChildNodes)
                {
                        ...
                        case "DECLARACION":
                            ...
                            if (dataType >= Tipo.INT && dataType <= Tipo.CLASE)
                            {
                                foreach (var varlst in ramaAux[1].ChildNodes)
                                {
                                    ...
                                    if (CaseSym.ContainsKey(varlst.Token.Text.ToLower()))
                                    {
                                    ...
                                    if (!ramaAux[ramaAux.Count - 1].Term.Name.Equals("OPER"))
                                    {
                                        ...
                                        this.CaseSym.Add(varlst.Token.Text.ToLower(), new Simbolo(pos, false, null, dataType));
                                    }
                                    else
                                    {
                                        //Opera el oper
                                        ...
                                        CaseSym.Add(varlst.Token.Text.ToLower(), ArrSym); Main.Imprimir(String.Format("La variable {0} ya ha sido declarada en este entorno: ({1},{2})", varlst.Token.Text.ToLower(), varlst.Token.Location.Line + 1, varlst.Token.Location.Column + 1));
                                    }
                                }
                            }
                            //Es un ARRAY
                            if (dataType >= Tipo.INTARR && dataType <= Tipo.CLASEARR)
                            {
                                ...
                                foreach (var varlst in ramaAux[1].ChildNodes)
                                {
                                    ...
                                    if (ramaAux.Count == 3)
                                    {
                                        ...
                                        //Añade el simbolo al entorno local
                                        CaseSym.Add(varlst.Token.Text.ToLower(), ArrSym);
                                    }
                                    else
                                    {
                                        //Opera el OPER
        ...
        /// <summary>
        /// Busca algun simbolo en el entorno actual, si no lo encuentra va al padre
        /// </summary>
        /// <param name="nombreVar"></param>
        /// <returns></returns>
        public new Simbolo BuscarSimbolo(string nombreVar)
        {
            return this.CaseSym.ContainsKey(nombreVar) ? this.CaseSym[nombreVar] : base.BuscarSimbolo(nombreVar);
        }
    }
}
