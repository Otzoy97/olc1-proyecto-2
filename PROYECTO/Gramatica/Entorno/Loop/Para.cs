...
namespace PROYECTO.Gramatica.Entorno.Loop
{
    class Para :  IEntorno
    {
        ...
        /// <summary>
        /// Especifica la condición a evaluar y el incremento a realizar
        /// </summary>
        public ParseTreeNode ForDec { get; }
        public ParseTreeNode ForCond { get; }
        public ParseTreeNode ForIncremento { get;  }
        /// <summary>
        ...
        public Para(ParseTreeNode forDeclaracion, ParseTreeNode forCondicion, ParseTreeNode forIncremento, ParseTreeNode acciones, IEntorno entornoPadre)
        {
            this.ForSym = new Dictionary<string, Simbolo>();
            this.ForTree = acciones;
            this.ForCond = forCondicion;
            this.ForDec = forDeclaracion;
            ForIncremento = forIncremento;
        ...
        public bool Ejecutar()
        {
            ...
            //Opera el nodo de declaración
            switch (ForDec.ChildNodes.Count)
            {
                case 2:
                    //asignación
                    //buscará el nombre de la variable del nodo +
                    var SimForRef = BuscarSimbolo(ForDec.ChildNodes[0].Token.Text.ToLower());
                    //Determina que no sea nulo y que sea tipo int
                    if (SimForRef.TipoDato != Tipo.INT || SimForRef.Dato == null)
                    {
                        Main.Imprimir(String.Format("Solo se admite valores tipo entero ({0},{1})", ForDec.ChildNodes[0].Token.Location.Line + 1, ForDec.ChildNodes[0].Token.Location.Column + 1));
                        return true;
                    }
                    //Ya encontró la variable , ahora asigna el nuevo valor
                    var SimNewForRef = operar.Interpretar(ForDec.ChildNodes[1]);
                    //Obligará que sea del mismo tipo que el SimForRef
                    if (SimForRef.TipoDato != SimNewForRef.TipoDato || SimNewForRef == null)
                    {
                        Main.Imprimir(String.Format("No se puede asignar la variable, los tipos no coinciden ({0},{1})", ForDec.ChildNodes[0].Token.Location.Line + 1, ForDec.ChildNodes[0].Token.Location.Column + 1));
                        return true;
                    }
                    //Ambos son del mismo tipo y se asigna el nuevo oper dato al simforref
                    SimForRef.Dato = SimNewForRef.Dato;
                    inicio = (int)SimNewForRef.Dato;
                    break;
                case 3:
                    //declaración
                    //opera el lado de oper
                    SimNewForRef = operar.Interpretar(ForDec.ChildNodes[2]);
                    //Obligará que sea del mismo tipo que el SimForRef
                    if (Tipo.INT != SimNewForRef.TipoDato || SimNewForRef == null)
                    {
                        Main.Imprimir(String.Format("No se puede asignar la variable, los tipos no coinciden ({0},{1})", ForDec.ChildNodes[0].Token.Location.Line + 1, ForDec.ChildNodes[0].Token.Location.Column + 1));
                        return true;
                    }
                    Posicion pos = new Posicion(ForDec.ChildNodes[0].Token.Location.Line + 1, ForDec.ChildNodes[0].Token.Location.Column + 1);
                    //Los tipos coinciden y no es nulo, procede a asignar el nuevo valor
                    Simbolo newForSym = new Simbolo
                    {
                        Dato = SimNewForRef.Dato,
                        TipoDato = Tipo.INT,
                        Posicion = pos,
                    };
                    inicio = (int)SimNewForRef.Dato;
                    //Crea la nueva variable en el diccionario local
                    this.ForSym.Add(ForDec.ChildNodes[1].Token.Text.ToLower(), newForSym);
                    break;
            }
            //Recorre el nodo de acciones
            try
            {
                while((bool)operar.Interpretar(ForCond).Dato)
                {
                    foreach (var nodeTree in ForTree.ChildNodes)
                    {
                            ...
                            case "DECLARACION":
                                ...
                                if (dataType >= Tipo.INT && dataType <= Tipo.CLASE)
                                {
                                    foreach (var varlst in ramaAux[1].ChildNodes)
                                    {
                                        ...
                                        if (ForSym.ContainsKey(varlst.Token.Text.ToLower()))
                                        {
                                        ...
                                        if (!ramaAux[ramaAux.Count - 1].Term.Name.Equals("OPER"))
                                        {
                                            ...
                                            this.ForSym.Add(varlst.Token.Text.ToLower(), new Simbolo(pos, false, null, dataType));
                                        }
                                        else
                                        {
                                            //Opera el oper
                                            ...
                                            ForSym.Add(varlst.Token.Text.ToLower(), ArrSym); Main.Imprimir(String.Format("La variable {0} ya ha sido declarada en este entorno: ({1},{2})", varlst.Token.Text.ToLower(), varlst.Token.Location.Line + 1, varlst.Token.Location.Column + 1));
                                        }
                                    }
                                }
                                //Es un ARRAY
                                if (dataType >= Tipo.INTARR && dataType <= Tipo.CLASEARR)
                                {
                                    ...
                                    foreach (var varlst in ramaAux[1].ChildNodes)
                                    {
                                        ...
                                        if (ramaAux.Count == 3)
                                        {
                                            ...
                                            //Añade el simbolo al entorno local
                                            ForSym.Add(varlst.Token.Text.ToLower(), ArrSym);
                                        }
                                        else
                                        {
                                            //Opera el OPER
                    ...
                    ///Opera el incremento
                    switch (ForIncremento.ChildNodes.Count)
                    {
                        case 1:
                            #region OPER
                            //Opera el nodo
                            operar.Interpretar(ForIncremento.ChildNodes[0]);
                            #endregion
                            break;
                        case 2:
                            #region VARIABLE SIMPLE
                            //Opera el nodo 1
                            Simbolo symOper = operar.Interpretar(ForIncremento.ChildNodes[1]);
                            //Busca la variable del nodo 0
                            Simbolo symRef = BuscarSimbolo(ForIncremento.ChildNodes[0].Token.Text.ToLower());
                            //Se asegura que ambos simbolos sean del mismo tipo de datos
                            //obviamente no pueden ser arrgelos
                            if (symOper.TipoDato != symRef.TipoDato)
                            {
