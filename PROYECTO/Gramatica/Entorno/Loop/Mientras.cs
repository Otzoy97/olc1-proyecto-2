...
namespace PROYECTO.Gramatica.Entorno.Loop
{
    class Mientras: IEntorno
    {
        ...
        public bool Ejecutar()
        {
            ...
            try
            {
                while ((bool)operar.Interpretar(this.WhileCond).Dato)
                {
                    //Recorre el nodo de acciones
                    foreach (var nodeTree in WhileTree.ChildNodes)
                    {
                            ...
                            case "DECLARACION":
                                ...
                                if (dataType >= Tipo.INT && dataType <= Tipo.CLASE)
                                {
                                    foreach (var varlst in ramaAux[1].ChildNodes)
                                    {
                                        ...
                                        if (WhileSym.ContainsKey(varlst.Token.Text.ToLower()))
                                        {
                                        ...
                                        if (!ramaAux[ramaAux.Count - 1].Term.Name.Equals("OPER"))
                                        {
                                            ...
                                            this.WhileSym.Add(varlst.Token.Text.ToLower(), new Simbolo(pos, false, null, dataType));
                                        }
                                        else
                                        {
                                            //Opera el oper
                                            ...
                                            WhileSym.Add(varlst.Token.Text.ToLower(), ArrSym); Main.Imprimir(String.Format("La variable {0} ya ha sido declarada en este entorno: ({1},{2})", varlst.Token.Text.ToLower(), varlst.Token.Location.Line + 1, varlst.Token.Location.Column + 1));
                                        }
                                    }
                                }
                                //Es un ARRAY
                                if (dataType >= Tipo.INTARR && dataType <= Tipo.CLASEARR)
                                {
                                    ...
                                    foreach (var varlst in ramaAux[1].ChildNodes)
                                    {
                                        ...
                                        if (ramaAux.Count == 3)
                                        {
                                            ...
                                            //Añade el simbolo al entorno local
                                            WhileSym.Add(varlst.Token.Text.ToLower(), ArrSym);
                                        }
                                        else
                                        {
                                            //Opera el OPER
